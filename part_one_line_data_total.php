...
$relax_array = array();
	/*抓取日期資料筆數*/
	if($date2!=null){
	$date_data_total = mysql_query("SELECT distinct (date) FROM total_data WHERE (stage = 1 or stage = 2 or stage = 3 or stage = 4 or stage = 5 or stage = 6) and account ='$id' and date>='$date2' order by no limit 21");
	}
	else{
	$date_data_total = mysql_query("(SELECT distinct (date) FROM total_data WHERE (stage = 1 or stage = 2 or stage = 3 or stage = 4 or stage = 5 or stage = 6) and account ='$id' order by no DESC limit 21)order by date");
	}
	/*日期有幾筆*/
...
/*For迴圈 Run日期筆數*/
	...
	$result=mysql_query("SELECT attention FROM total_data WHERE (stage = 2 or stage = 3 or stage = 5 or stage = 6) and date = '$day_atotal_atotal' and account = '$id'");
	...
	//$attdata = 100-($following / $last_acount*100);
	$last_asum =0;
	$last_acount=0;
	//$following = 0; 
	echo $attdata."<BR>";
...
/*For迴圈 Run日期筆數*/
	...
	$result=mysql_query("SELECT relax FROM total_data WHERE (stage = 1 or stage = 3 or stage = 4 or stage = 6) and date = '$day_mtotal_mtotal' and account = '$id'");
...
$level_array = array($high_level_line_data);
$level_array2 = array($low_level_line_data);

//將設定參數傳入繪製統計圖
echo part_chart('null','null');
echo chart('line', $title, $x_array, $key_array, $data_array);
?>
