...
$relax_array = array();
	/*抓取日期資料筆數*/
	if($date2!=null){
	$date_data_total = mysql_query("SELECT  distinct (date) FROM total_data WHERE (stage = 19 or stage = 20 or stage = 21 ) and account ='$id' and date>='$date2' order by no limit 21");
	}
	else{
	$date_data_total = mysql_query("(SELECT distinct (date) FROM total_data WHERE (stage = 19 or stage = 20 or stage = 21 ) and account ='$id' order by no DESC limit 21)order by date");
	}
	/*日期有幾筆*/
...
/*For迴圈 Run日期筆數*/
	...
	$result=mysql_query("SELECT attention FROM total_data WHERE (stage = 20 or stage = 21 ) and date = '$day_atotal_atotal' and account = '$id'");
...
/*For迴圈 Run日期筆數*/
	...
	$result=mysql_query("SELECT relax FROM total_data WHERE (stage = 19 or stage = 21) and date = '$day_mtotal_mtotal' and account = '$id'");
